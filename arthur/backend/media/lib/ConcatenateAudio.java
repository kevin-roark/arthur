...
package arthur.backend.media.lib;
...
public class ConcatenateAudio {
	...
	public static void main(String[] args) {
		if (args.length < 3) {
			out.println("Concatent two files.  The destination " + "format will be guessed from the file extention.");
			out.println("");
		...
		for (int i = 0; i < 2; i++) {
			container.open(args[i], Type.READ, null);
			
			System.out.println("File: " + args[i]);
			System.out.println("Bit rate (bit/s): " + container.getBitRate());
			System.out.println("Duration (secs): " + container.getDuration() / (1000*1000));
...
System.out.println("Running");
		...
		while (reader1.readPacket() == null)
			;
System.out.println("First file ready.");				
	...
	static class MediaConcatenator extends MediaToolAdapter {
		...
		public void onAudioSamples(IAudioSamplesEvent event) {
			...
			if (this.audioResampler == null) {
				...
				this.audioResampler = IAudioResampler.make(2, samples.getChannels(), outputSampleRate, samples.getSampleRate());
			}
			IAudioSamples out = samples;
			if (samples.getNumSamples() > 0 && samples.getSampleRate() != outputSampleRate) {
				out = IAudioSamples.make(samples.getNumSamples(), samples.getChannels());
